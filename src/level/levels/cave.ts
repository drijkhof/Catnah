...
/** A row of charms, which is how a dead end says it was worth walking into. */
function charms(grid: Grid, x: number, y: number, count: number): void {
...
function build(): string[] {
  ...
  charms(grid, startOf(0) + 8, floors[0] - 1, 3);
  ...
  shelf(2, 3, 3);
  shelf(2, 8, 5);
  charms(grid, startOf(2) + 8, floors[2] - 6, 3);
  ...
  charms(grid, startOf(4) + 1, floors[4] - 1, 3);
  ...
  shelf(6, 2, 4);
  shelf(6, 7, 3);
  charms(grid, startOf(6) + 2, floors[6] - 5, 3);
  ...
  shelf(10, 3, 3);
  shelf(10, 8, 5);
  charms(grid, startOf(10) + 8, floors[10] - 6, 3);
  ...
  shelf(13, 8, 3);
  spider(13, 6);
  charms(grid, startOf(13) + 2, floors[13] - 5, 3);

  pillar(14, 4, 4);
  spider(14, 10);

  // The bottom, and the only door in the game that leads further down.
  charms(grid, startOf(15) + 3, floors[15] - 1, 3);
  put(grid, startOf(15) + 10, floors[15] - 1, 'E');

  // --- the short dead ends ------------------------------------------------
  // Each climbs out of the main run and stops. They are worth charms and
  ...
  const deadEnd = (step: number, at: number, length: number): void => {
    ...
    charms(grid, x + length - 4, mouth - 2, 3);
  };
  ...
  charms(grid, lairX + 14, lairFloor - 1, 3);
